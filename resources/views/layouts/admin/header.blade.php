<header
    ...
    <div class="flex items-center gap-4">
        <button @click="sidebarOpen = !sidebarOpen"
            ...
            <svg class="w-5 h-5 text-gray-500 group-hover:text-[#8B2452]" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
    ...
    class="relative p-2 text-gray-500 hover:text-[#8B2452] hover:bg-[#8B2452]/5 rounded-lg transition-all duration-200"
>
    ...
    @php
        $count = \App\Models\Notification::where(function ($query) {
    ...
    @if($count)
        <span class="absolute top-1 right-1 bg-red-500 text-white rounded-full text-[10px] min-w-[18px] h-[18px] flex items-center justify-center">
            {{ $count }}
        </span>
    @endif
</a>
        ...
        <div class="relative" x-data="{ profileOpen: false }">
            <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false"
                class="flex items-center gap-3 p-1 rounded-full hover:bg-gray-50 transition-all duration-200 border border-transparent hover:border-gray-200">
                @php
                ...
                @else
                    <div
                        class="w-9 h-9 rounded-full flex items-center justify-center bg-[#8B2452] text-white font-bold text-sm shadow-sm">
            ...
            <div x-show="profileOpen" x-cloak x-transition:enter="transition ease-out duration-100"
                ...
                <a href="{{ route('admin.profile') }}"
                    class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#8B2452]/5 hover:text-[#8B2452] transition-colors duration-200">
