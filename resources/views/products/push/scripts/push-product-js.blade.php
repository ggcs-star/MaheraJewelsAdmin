...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Purchase Order Data
    const purchaseOrderData = window.purchaseOrderData || {};
    ...
    // Auto-select product if productId exists
    ...
    availableVariants.forEach(v => {
        ...
        const poData = window.purchaseOrderData ? window.purchaseOrderData[v.id] : null;
        const hasPO = poData !== null && poData !== undefined;

        let poBadge = '';
        if (hasPO) {
            poBadge = `<span class="badge bg-info text-dark ms-1" title="PO: ${poData.po_number || 'N/A'}">📦 PO</span>`;
        }

        const statusBadge = variantPlatformData[v.id] && Object.keys(variantPlatformData[v.id]).length
        ...
        variantsBody.insertAdjacentHTML('beforeend', `
            <tr class="text-center">
                ...
                <td>${v.variant_type} ${poBadge}</td>
                <td>${v.variant_value}</td>
                <td>${colorDisplay}</td>
                <td>${v.sku_suffix ?? '-'}</td>
                <td class="fw-bold text-success">${hasPO ? poData.quantity : v.quantity}</td>
                <td class="fw-bold text-primary">₹ ${hasPO ? poData.purchase_price : (v.purchase_price ?? 0)}</td>
    ...
    document.querySelectorAll('.configure-variant-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (!formVariantInput) {
                ...
                formVariantInput.type = 'hidden';
                formVariantInput.id = 'formVariantId';
                formVariantInput.name = 'variant_id';
                document.querySelector('#pushProductForm').appendChild(formVariantInput);
            }
            formVariantInput.value = activeVariantId;
...
document.addEventListener('input', function(e) {
    if (e.target.classList.contains('platform-qty-input')) {
        const platformId = e.target.id.split('_').pop();
        const qty = parseInt(e.target.value) || 0;
        const available = parseInt(e.target.closest('.row').querySelector('.available-stock')?.innerText) || 0;
        const errorDiv = document.getElementById('stockError_' + platformId);
        const remainingSpan = e.target.closest('.row').querySelector('.remaining-stock');
        ...
        if (qty > available) {
            ...
            showToast('⚠️ Quantity cannot exceed available stock: ' + available, 'danger');
        } else {
            if (errorDiv) errorDiv.style.display = 'none';
            e.target.style.borderColor = '';
            if (remainingSpan) {
                remainingSpan.innerText = available - qty;
                remainingSpan.style.color = qty > 0 ? '#1a7a3a' : '';
            }
        }
